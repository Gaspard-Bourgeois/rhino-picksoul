...
def decompose_block_instance(obj_id, robot_name, instance_index):
    ...
    for item in targets:
        ...
        # Stockage de la pose de repos absolue (pose neutre, angles tous à 0°)
...
def resolve_input():
    ...
    Cas B – pas de pré-sélection :
        Demande le nom du bloc, un point d'insertion, insère et décompose.
    """
    # ── Cas A ────────────────────────────────────────────────────────────────
...
def _find_previous_joint0_xform(robot_name, instance_index):
    ...
    de son joint 0 pour réutiliser la position.
    """
    if instance_index <= 1:
        return None
...
def read_stored_angles(group):
    """
    Lit les KEY_ANGLE stockés sur chaque joint.
    Retourne {joint_idx: float} — angles relatifs au repère parent.
    """
    stored = {}
...
def compute_and_apply(group, angles_deg, T0_rest, robot_name, instance_index):
    """
    Cinématique série depuis les poses de repos T0_rest (pose neutre, angles à 0°).

    Les angles sont RELATIFS au repère parent (bloc de référence).
    ...
    On repart de T0_rest avant chaque application pour éviter toute dérive.
    """
    # Transformées locales parent→enfant depuis les poses neutres
    T_local = {0: T0_rest[0]}
    ...
    # Transformées monde avec angles relatifs
    W = {0: T0_rest[0]}   # base fixe (joint 0 sans angle)
    for i in range(1, NB_JOINTS):
    ...
    for i in range(NB_JOINTS):
        ...
        # Remise à la pose neutre avant application du delta
...
class RobotPoseDialog(ef.Dialog):
    ...
    def __init__(self, group, stored_angles, joint_limits,
        ...
        # Angles initiaux = stored_angles (relatifs au repère parent)
        ...
        self._build_ui()
        # Pas de _refresh_viewport() ici : main() applique déjà les stored_angles
        # avant d'instancier le dialogue.
...
def main():
    ...
    # 2. Angles stockés (relatifs au repère parent) et limites articulaires
    stored_angles = read_stored_angles(group)
    joint_limits  = read_joint_limits(group)

    # 3. T0_rest = pose NEUTRE (angles tous à 0°), obligatoirement depuis KEY_REST_XFORM.
    #    Cette transformée ne doit jamais être calculée depuis la xform courante,
    #    qui peut déjà être tournée.
    T0_rest = {}
    for i in range(NB_JOINTS):
        ...
        if xf is None:
            rs.MessageBox(
                "RestXform manquante pour le segment {}.\n"
                "Le bloc doit être réinséré pour initialiser la pose neutre.".format(i),
                0, "Erreur")
            return
        T0_rest[i] = xf

    # 4. Capture de l'état courant avant édition (position + angles)
    #    pour restauration exacte en cas d'annulation.
    T0_before     = {}
    angles_before = [stored_angles.get(i, 0.0) for i in range(NB_JOINTS)]
    for i in range(NB_JOINTS):
    ...
    # 5. Application des stored_angles avant ouverture du dialogue.
    #    Garantit que le viewport part de la pose réelle (reprise d'édition),
    #    et que curseurs + viewport sont cohérents dès l'affichage.
    rs.EnableRedraw(False)
    compute_and_apply(group, angles_before, T0_rest, robot_name, instance_index)
    rs.EnableRedraw(True)
    sc.doc.Views.Redraw()
    ...
    if dlg.validated:
        ...
        if ok:
            ...
            # Sélection : joints 0–6 + instances Pose de même KEY_LEVEL0
