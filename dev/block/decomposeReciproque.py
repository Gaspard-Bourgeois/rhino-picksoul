...
def explode_any(obj_id):
    # TODO: fonction à remplacer par une fonction qui n'explose que les instances
    """
...
def decompose_reciproque():
    ...
    for obj_id in object_ids:
        ...
        if rs.IsBlockInstance(obj_id):
            ...
            exploded_items = list(rs.ExplodeBlockInstance(obj_id))
            
            origin_obj = None
            # On cherche si un sous-bloc "Pose" est déjà présent à l'origine
            # TODO : on insert dans tous les cas un bloc pose à l'origine
            ...
            # TODO : Remplacer par un usertext du type BlocNameLevel_X = block_name avec X le niveau d'imbrication actuel
            rs.SetUserText(origin_obj, "OriginalBlockName", block_name)
            
            # Groupement du résultat
            # TODO: on récupère le niveau hiérarchique actuelle des instances à décomposer à partir des usertexts du type BlocNameLevel_X = block_name#Y, X le niveau d'imbrication de bloc, Y l'indice de l'instance de bloc
            # TODO: l'indice de l'instance décomposée est unique par block_name dans le document, tous les groupes de la même instances décomposé ont un indice d'instance différent
            # TODO: on ne groupe plus les résultats
            group = rs.AddGroup()
